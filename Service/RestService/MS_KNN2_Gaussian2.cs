...
{
    public class MS_KNN2_Gaussian2
    {
        ...
        private void kNN_Cal(Decimal[] rssi, double[] sum, DataColumnCollection column, DataRowCollection rows, double[,] temp, double[] comp, int[] Rm, DataTable knn)
        {
            ...
            foreach (DataRow dr in rows)
            {
                for (i = 0; i < 5; i++)
                {
                    temp[0, j] += //Math.Abs(Math.Pow(Convert.ToDouble(rssi[i]), 2) - Math.Pow(Convert.ToDouble(dr[i + 1]), 2));
                    Math.Abs(Convert.ToDouble(rssi[i]) - Convert.ToDouble(dr[i + 1]));
                }
        ...
        private void Result(double[] new1, double[] G_Avg, int[] Rm, DataTable knn, double[,] tempd)
        {
            ...
            foreach (DataRow kr in knn.Rows)
            {
                for (j = 0; j < 5; j++)
                {
                    new1[i] += //Math.Sqrt(Math.Pow(Math.Pow(G_Avg[j], 2) - Math.Pow(Convert.ToDouble(kr[j + 1]), 2), 2));
                    Math.Sqrt(Math.Pow((G_Avg[j] - Convert.ToDouble(kr[j + 1])), 2));
                }
                i++;
            }
            i = 0;
