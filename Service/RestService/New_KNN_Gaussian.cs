    ...
    public class New_KNN_Gaussian
    {
        ...
        private void mDiv_Cal(int[] Rm, double[] mDiv, double[] mAvg, DataTable knn)
        {
            ...
            for (i = 0; i < 5; i++)
            {
                foreach (DataRow kr in knn.Rows)
                {
                    mDiv[i] += Math.Pow(Convert.ToDouble(kr[j + 1]) - mAvg[i], 2);
                }
                mDiv[i] = Math.Sqrt(mDiv[i] / 2);
                j++;
            }
        }
        ...
        private void Gauss_Cal(double[] Gauss, DataTable knn, int[] Rm, double[] mDiv, double[] mAvg)
        {
            ...
            for (i = 0; i < 5; i++)
            {
                foreach (DataRow kr in knn.Rows)
                {
                    ...
                    //가우스함수식 kr == knn으로 선택된 최근접 5 데이터, mAvg == knn 5개 데이터 평균, mDiv == 표준편차(Math.Pow 거듭제곱 반환시 분산이됨)
                    j++;
                }
            }
            for (i = 0; i < 3; i++)
