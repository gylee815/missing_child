...
{
    public class New_KNN_Gaussian2_K6
    {
        ...
        private void mDiv_Cal(double[] mDiv, double[] mAvg, DataTable knn)
        {
            ...
            for (i = 0; i < 5; i++)
            {
                foreach (DataRow kr in knn.Rows)
                {
                    mDiv[i] += Math.Sqrt(Math.Pow(Convert.ToDouble(kr[i + 1]) - mAvg[i], 2) / 5);
                }
            }
        }
