...
{
    public class New_KNN_Gaussian_K5
    {
        ...
        private void mDiv_Cal(double[] mDiv, double[] mAvg, DataTable knn)
        {
            int i = 0;
            ...
            for (i = 0; i < 5; i++)
            {
                foreach (DataRow kr in knn.Rows)
                {
                    mDiv[i] += Math.Pow(Convert.ToDouble(kr[i + 1]) - mAvg[i], 2);
                }
                mDiv[i] = Math.Sqrt(mDiv[i] / 4);
            }
        }

        private void Gauss_Cal(double[] Gauss, DataTable knn, double[] mDiv, double[] mAvg)
        {
            int i = 0;
            int j = 0;
        ...
        private void G_Avg_Cal(double[] G_Avg, double[,] f, double[] G_Sum, double[] G_add, double[] Gauss, DataTable knn)
        {
            int i = 0;
            int j = 0;
            int u = 0;
            ...
            for (i = 0; i < 5; i++)
            {
                for (j = 0; j < 5; j++)
                {
                    G_Sum[i] += f[i, j];  //데이터 * 가중치 들의 합
                    G_add[i] += Gauss[y];   //적용된 가중치들 끼리의 합
                    y++;
                }
        ...
        public int[] Calc_Result(Decimal[] ARR_AP, DataTable table)
        {
            ...
            mDiv_Cal(mDiv, mAvg, knn);

            double[] Gauss = new double[25];
            Gauss_Cal(Gauss, knn, mDiv, mAvg);
            //==========================================가중평균=============================================//
            double[] G_Avg = new double[5];     //  최죵적인 결과 가중평균을 통해 예측 수신세기 도출                 
            double[,] f = new double[5, 5];     //  데이터 * 가중치 
            double[] G_Sum = new double[5];     //  (데이터 * 가중치) 들의 합
            double[] G_add = new double[5];     //  가중치 합
            G_Avg_Cal(G_Avg, f, G_Sum, G_add, Gauss, knn);
            //==================================== 예측값으로 kNN 구하기 ====================================//
            double[] new1 = new double[5];
            double[,] tempd = new double[4, 5];
