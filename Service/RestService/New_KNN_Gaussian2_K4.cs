...
{
    public class New_KNN_Gaussian2_K4
    {
        ...
        public static void mDiv_Cal(double[] mDiv, double[] mAvg, DataTable knn)
        {
            int i = 0;
            ...
            for (i = 0; i < 5; i++)
            {
                foreach (DataRow kr in knn.Rows)
                {
                    mDiv[i] += Math.Sqrt(Math.Pow(Convert.ToDouble(kr[i + 1]) - mAvg[i], 2) / 3);
                }
            }
        }
